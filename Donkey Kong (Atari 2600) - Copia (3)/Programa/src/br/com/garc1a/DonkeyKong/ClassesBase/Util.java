...
public class Util {
	public static int diferencaPosY(Elemento a , Elemento b) {
		if(a.getPy()-b.getPy() < 0) {
			return -1;
		}
		return 1;
	}
	...
	public static boolean colideChao(Elemento a, Elemento b) {
		if (!a.isAtivo())
			return false;
	...
	//TODO FAZER AS COLISOES COM OS CA RA LE O DOS CHAOS, mas esta tudo encaminhado :)
	public static boolean colideY(Elemento a, Elemento b) {
		if (!a.isAtivo() || !b.isAtivo())
			return false;
		
		if (a.getPy() + a.getAltura() >= b.getPy() && a.getPy() <= b.getPy() + b.getAltura()) {
			return true;
		}
