...
public class JogoCenario extends CenarioPadrao{
	...
	public void carregar() {
		...
		for (int linha = 0; linha < nivelSelecionado.length; linha++) {
			for (int coluna = 0; coluna < nivelSelecionado[0].length; coluna++) {
				System.out.println(nivelSelecionado[linha][coluna]);
	...
	public void acoesMario() {
		...
		//LooP para colisoes mario
		for (Elemento e : nivel) {
			if (e == null)
				break;
			
			if (Util.colide(mario, e) && (e.getIndicacao() == 'B' || e.getIndicacao() == 'I')){
				pulou = false;
				setColidiu(true);
				peso = true;
				paraBaixo = false;
			...
			}if(Jogo.controleTecla[0] && Util.colide(mario, e) && (e.getIndicacao() == 'E' || e.getIndicacao() == 'F')  && !pulou){
				peso = false;
				setColidiu(true);
				mario.setPy(mario.getPy() - mario.getVel()/2);
				setEscada(true);
				
			}if(Jogo.controleTecla[1] && Util.colide(mario, e) && (e.getIndicacao() == 'E' || e.getIndicacao() == 'F')  && !pulou){
				setSubindo(true);
				peso = false;
				setColidiu(true);
				setEscada(true);
