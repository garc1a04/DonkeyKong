...
public class Menu extends Texto {
	...
	public void desenha(Graphics2D g) {
		...
		g.setColor(getCor());
		super.desenha(g,"DONKEY KONG", getPx()+10, getPy()- 35+ getAltura());
		super.desenha(g, String.format("%s: <%s>", getRotulo(), opcoes[idx]), getPx(), getPy()+ getAltura());

		if (selecionado)
			g.drawLine(getPx(), getPy() + getAltura() + 10, getPx() + getLargura(), getPy() + getAltura() + 10);
			g.drawLine(getPx(), getPy()-getPy()/10+ getAltura() + 5, getPx()+10 + getLargura(), getPy()-getPy()/10 + getAltura()+ 5);
